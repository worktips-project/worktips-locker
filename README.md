# Worktips Locker - a lightweight web wallet for Worktips based on OpenMonero 
...
demonstrate how to provide Worktips related JSON REST service. For this purpose,
 a service called Worktips Locker was developed.


Worktips Locker is an open source implementation of backend of
https://mymonero.com/ adapted for Worktips. The frontend, which includes HTML, CSS, JavaScript, was adapted
from (and originally developed by) https://mymonero.com/.

Worktips Locker's backend is open sourced, free
...
The Worktips Locker consists of four components that need to be setup for it to work:
 ...
 from the Worktips Locker into the Worktips network.
...
Worktips Locker is not as fast as MyMonero.
...
Below are example and basic instructions on how to setup up and run Worktips Locker on Ubuntu 16.04.
...
git clone --recursive https://github.com/worktips-project/worktips-core
...
#### Compilation of the Worktips Locker (don't run it yet)

Once Worktips was downloaded and compiled, we can download Worktips Locker and compile it.
...
git clone https://github.com/worktips-project/worktips-locker.git
...
```
docker run --name worktipshtml -p 80:80 -v /home/username/worktipslocker/html:/usr/share/nginx/html:ro -d nginx
```
...
Before running `worktips-locker`:
...
```bash
./worktips-locker -s -c /path/to/config.json
```


## Worktips Locker JSON REST API

Example JSON REST requests and their responses of [Worktips Locker](https://github.com/worktips-project/worktips-locker) are provided below. The long term goal
...
the Worktips Locker API does not fully conform to that.

#### get_version

Get version of the Worktips Locker, its API and Worktips.
...
Login an existing or new user into Worktips Locker
