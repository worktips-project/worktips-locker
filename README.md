...
Download `db.sql` provided and setup the `loki_wallet` database. `db.sql` script will
drop current `loki_wallet` if exist. So don't run it, if you have already some important information
in the `loki_wallet` database.
...
```bash
sudo mkdir /var/www/html/loki-locker
sudo cp -rvf ../html/* /var/www/html/loki-locker/
```

Setup document root in `lighttpd.conf` into loki-locker folder
...
```bash
server.document-root    = "/var/www/html/loki-locker"
```
